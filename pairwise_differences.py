...
def plot_one_pairwise_distribution(config: FileConfiguration, pop: str) -> None:
    freq_table = pd.read_csv(f"{config.output_path}/pairwise_differences/frequency_pairwise_distances_{pop}.csv",
                             sep=";")
    ...
    plt.xticks(list(np.linspace(0, 2*len(config.loci_list), 2*len(config.loci_list) + 1)))
    plt.tight_layout()
    plt.savefig(f"{config.output_path}/pairwise_differences/plots/plot_pairwise_frequencies_{pop}.pdf")
    plt.close()


def plot_two_pairwise_distributions(config: FileConfiguration, pop1: str, pop2: str) -> None:
    # get distances
    freq_table1 = pd.read_csv(f"{config.output_path}/pairwise_differences/frequency_pairwise_distances_{pop1}.csv",
                              sep=";")
    freq_table2 = pd.read_csv(f"{config.output_path}/pairwise_differences/frequency_pairwise_distances_{pop2}.csv",
                              sep=";")
    ...
    ax.set_xticks(list(np.linspace(0, 2*len(config.loci_list), 2*len(config.loci_list) + 1)))
    ax.legend()
    ax.set_title(pop1 + " - " + pop2)
    plt.tight_layout()
    plt.savefig(f"{config.output_path}/pairwise_differences/plots/plot_pairwise_frequencies_{pop1}_{pop2}.pdf")
    plt.close()
...
def plot_intra_population_distances(config: FileConfiguration, pop1__pop2: str) -> None:
    ...
    df_selection = config.genotypes_data_pairwise[config.genotypes_data_pairwise.Population.isin([pop1, pop2])]
    calculate_pairwise_differences(
        config, config.selection_name + f"_{pop1__pop2}", df_selection
    )
    diff_matrix = pd.read_csv(f"{config.output_path}/pairwise_differences/pairwise_differences_{config.selection_name}_{pop1__pop2}.csv",
                              sep=";")
    diff_matrix["pop_cat"] = diff_matrix.Sample.apply(lambda x: config.dict_pop_samples[x])
...
def plot_pairwise_distances_one_by_one(config: FileConfiguration) -> None:
    if config.agg_type == "all":
        ...
        for _pop, _year in zip(selection.Population, selection.Year):
            plot_one_pairwise_distribution(config, config.selection_name + f"_{_pop}_{_year}")
    else:  # subcat
        for _pop, _year, _sub in zip(
                config.pops_to_select.Population, config.pops_to_select.Year, config.pops_to_select.Subcategory
        ):
            ext = get_extension_if_subcat(_sub)
            plot_one_pairwise_distribution(config, config.selection_name + f"_{_pop}_{_year}{ext}")


def plot_pairwise_distances_two_by_two(config: FileConfiguration) -> None:
    if config.agg_type == "all":
        print("'all' aggregation type selected - only got one distribution, so cannot plot two.")

    elif config.agg_type == "pops":
        possible_combinations = get_combination_duets(config.pops_to_select.Population.unique())
        for _pop1, _pop2 in possible_combinations:
            plot_two_pairwise_distributions(config, config.selection_name + f"_{_pop1}",
                                            config.selection_name + f"_{_pop2}")
    elif config.agg_type == "pop_years":
        selection = config.pops_to_select[["Population", "Year"]].drop_duplicates()
        selection["pop_year"] = selection.apply(lambda x: x.Population + "__" + str(x.Year), axis=1)
        ...
        for _ele1, _ele2 in possible_combinations:
            ...
            plot_two_pairwise_distributions(config, config.selection_name + f"_{_pop1}_{_year1}",
                                            config.selection_name + f"_{_pop2}_{_year2}")

    else:  # subcat
        selection = config.pops_to_select.copy()
        selection["pop_year_sub"] = selection.apply(
            lambda x: x.Population + "__" + str(x.Year) + get_extension_if_subcat(x.Subcategory),
            axis=1)
        ...
        for _ele1, _ele2 in possible_combinations:
            ...
            plot_two_pairwise_distributions(config, config.selection_name + f"_{_pop1}_{_year_ext1}",
...
def plot_pairwise_distances_all_intra_populations(config: FileConfiguration) -> None:
    if config.agg_type == "pops":
        ...
        print("plot_pairwise_distances_all_intra_populations - available only for 'pops' aggregation type")
